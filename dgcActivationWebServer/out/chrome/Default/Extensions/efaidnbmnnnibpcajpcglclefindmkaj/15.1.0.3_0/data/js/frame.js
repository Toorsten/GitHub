...
function doAcrobat() {
	...
	if (request.version === SETTINGS.READER_VER) {
        analytics(events.TREFOIL_PDF_READER);
    } else {analytics(events.TREFOIL_PDF_ACROBAT);
        }
...
function setStatus(r) {
	...
	if (r.version === SETTINGS.ERP_READER_VER) {
	...
	if (request.version === SETTINGS.READER_VER) {
        $("#web2pdfOpenButtonText").val(
            util.getTranslation("web2pdfOpenButtonTextReader")
        );
    }
